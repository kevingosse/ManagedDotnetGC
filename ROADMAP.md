...
| M7 ◐ | Tuning war | Beat stock on target workloads 1–3; publish reproducible results. **Matrix rerun post-M6 (`m6s3-fairness`, 2026-07-06)**: beats WKS on all four (0.74–0.86×), even with default SVR on lohmix (1.01×) and within 9–11% elsewhere, beats SVR+BGC on pinheavy (0.88×); the tuned SVR-h8 row still leads everywhere but the gap narrowed to 1.17–1.50× (was 1.14–1.71×). Memory parity improved most: 1.12× of default SVR at a 4.4 GB cap on pinheavy (was 1.47×). Open: the memory exchange rate (uncapped peak 6.6–8.5 GB vs stock 1.1–4.4 GB — now the top item), lohmix run-scan cost, young-pause p50 |
