...
| M7 ◐ | Tuning war | Beat stock on target workloads 1–3; publish reproducible results. **Fairness matrix recorded 2026-07-06** (results/perf-history.md): beats WKS everywhere and default SVR-32 + SVR+BGC on 3 of 4 scenarios; loses all 4 to the tuned SVR-h8 row (1.03–1.33×) which uses *half* our memory — at enforced memory parity we're 1.1× (pinheavy) to 2.0× (lohmix). Open: lohmix (only loss vs default SVR), the memory exchange rate, young-pause p50 |
