...
| M9.1 ✅ | **Boost cap 192 MB** — residual-gap profile closes the loop | Same-sitting ETW diff (EtlCpu) found GC CPU nets to zero vs stock; three clean negatives (nursery size 256–512, temporal zeroing, GC frequency) all RPS-flat down to a 256 MB budget, so the 448 MB boost bought nothing — capped at 192 (budget 256, demand 320): **fortunes WS 983→683 MB (below stock-h8 731), queries 448 MB, updates 429 MB, RPS unchanged 0.95–0.98×** vs same-sitting tuned stock-svr-h8; gates 56/56, GcStress ×3 (`70be506`, results/2026-07-07-m9-residual-profile.md) |
| M9.2 ✅ | **Probe-and-anchor boost controller** — first real-app throughput win | OrchardCore exposed the M9.1 absolute gates reading real apps backwards (18 MB concurrency-bound survivors > 16 MB shrink gate, 12 ms fixed pause > 10 ms cheap gate → budget pinned, 15% wall STW). Replaced with a probe: every grow anchors the survivor mass that justified it; survivors exceeding anchor×1.5+8 MB revert and mute 30 s — measures d(survivors)/d(budget), one fixed point per workload; cheap-pause gate deleted, floor bound reads durable live. **OrchardCore homepage 5266 rps = 1.055× pinned stock-h8 (was 0.95×), 0.96× DATAS, p50 parity, STW halved**; trade: WS ~1250 MB (1.34× h8) — WS now the weak real-app axis. Gates 56/56, GcStress ×3, TFB + mixed + soak unregressed (results/2026-07-07-m9.2-probe-controller.md) |
